...
func (db *DB) IndexProduct(pageURL string, p *ProductRecord) error {
	...
	if _, err := tx.Exec(`
		INSERT INTO products (page_url, name, description, price, currency, availability, image, extraction_method, raw_json)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
	...
	`, pageURL, p.Name, p.Description, price, p.Currency, p.Availability, p.Image, p.ExtractionMethod, p.RawJSON); err != nil {
