...
func TestCrawler_BasicBFSAndExtraction(t *testing.T) {
	...
	c := New(testCrawlConfig("sitedex-test"), testChunkConfig(), config.LLMExtractorConfig{}, t.TempDir(), writer, nil, nil)
...
func TestCrawler_ETagRevalidationSkipsUnchangedOnSecondCrawl(t *testing.T) {
	...
	c1 := New(testCrawlConfig("sitedex-test"), testChunkConfig(), config.LLMExtractorConfig{}, dataDir, writer, nil, nil)
	...
	c2 := New(testCrawlConfig("sitedex-test"), testChunkConfig(), config.LLMExtractorConfig{}, dataDir, writer, nil, nil)
...
func TestCrawler_MaxPagesLimit(t *testing.T) {
	...
	cfg.MaxPages = 3
	writer := newRecordingWriter()
	c := New(cfg, testChunkConfig(), config.LLMExtractorConfig{}, t.TempDir(), writer, nil, nil)
